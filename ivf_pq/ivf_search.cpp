...
void parallel_search (int nsq, int k, int comm_sz, int threads, long long tam, MPI_Comm search_comm, char *dataset, int w){
	...
	while(1){
		...
		int num_q, local_threads=1,last_local_thread;
		
		MPI_Bcast(&num_q, 1, MPI_INT, 0, search_comm);
		
		if(threads> num_q){
			local_threads = threads/num_q;
			last_local_thread = local_threads+threads%num_q;
			threads = num_q;
		}
			...
			while(1){
				...
				if(my_omp_rank==threads){
					...
					for(int i=my_omp_rank; i<residual.n/w; i+=threads){
						...
						for(int j=0; j<w; j++){
							dis_t q;
							q = ivfpq_search(ivf, &residual.mat[0]+(i*w+j)*residual.d, ivfpq.pq, coaidx[i*w+j], &g1, &g2, local_threads);
...
dis_t ivfpq_search(ivf_t *ivf, float *residual, pqtipo pq, int centroid_idx, double *g1, double *g2, int threads){
	dis_t q;
	int ds, ks, nsq;
	...
	for (int query = 0; query < nsq; query++) {
		...
		compute_cross_distances_thread(ds, 1, distab.d, &residual[query*ds], &pq.centroids[query*ks*ds], distab_temp,threads);
	...
	#pragma omp parallel for num_threads(threads) schedule(dynamic)
		for(i=0; i<lim; i++){
			...
			for(int j=0; j<ivfpq.coarsek; j++){
				...
				#pragma omp critical
				{
					sprintf(name_arq, "/pylon5/ac3uump/freire/ivf/ivf_%d_%lld_%d.bin", ivfpq.coarsek, tam, j);
