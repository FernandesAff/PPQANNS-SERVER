...
mat pq_test_load_train(char* dataset, int tam){
	...
	if(strcmp(dataset, "siftsmall")==0){
		...
		strcat (ftrain,"siftsmall/siftsmall_learn.fvecs");
		vtrain.n=25000;
		vtrain.d=128;
		...
		strcpy (ftrain, BASE_DIR);
		strcat (ftrain,"flickr/flickr_learn_50000.fvecs");
		vtrain.n=50000;
		vtrain.d=960;
		vtrain.mat= fmat_new (vtrain.d, vtrain.n);
	}
...
matI pq_test_load_gdn(char* dataset, int tam){
	...
	if(strcmp(dataset, "siftsmall")==0){
		...
		strcat (fgroundtruth,"siftsmall/siftsmall_gnd.ivecs");
		vids_gnd.n=100;
		vids_gnd.d=100;
		...
		strcpy (fgroundtruth, BASE_DIR);
		strcat (fgroundtruth,"flickr/flickr_learn_50000.fvecs");
		vids_gnd.n = 50000;
		vids_gnd.d = 960;
		ids_gnd = (int*) calloc(vids_gnd.n, sizeof(int));
	}
...
mat pq_test_load_query(char* dataset, int threads){
	...
	if(strcmp(dataset, "siftsmall")==0){
		...
		strcat (fquery,"siftsmall/siftsmall_query.fvecs");
		vquery.n=10000;
		vquery.d=128;
		...
		strcpy (fquery, BASE_DIR);
		strcat (fquery,"siftbig_1_query.bvecs");
		vquery.n=1;
		vquery.d=128;
		vquery.mat= fmat_new (vquery.d, vquery.n);
	}
		...
		strcpy (fquery, BASE_DIR);
		strcat (fquery,"flickr/flickr_query.fvecs");
		vquery.n=50;
		vquery.d=960;
		vquery.mat= fmat_new (vquery.d, vquery.n);
	}
...
mat pq_test_load_base(char* dataset, int offset, int my_rank){
	...
	if(strcmp(dataset, "siftsmall")==0){
		...
		strcat (fbase,"siftsmall/siftsmall_base.fvecs");
		vbase.n=10000;
		vbase.d=128;
		...
		strcpy (fbase, BASE_DIR);
		strcat (fbase,"flickr/flickr_base_50000.fvecs");
		vbase.n=50000;
		vbase.d=960;
		vbase.mat= fmat_new (vbase.d, vbase.n);
	}
